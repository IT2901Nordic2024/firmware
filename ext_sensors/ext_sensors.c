...
static struct sensor_trigger adxl362_sensor_trigger_motion = {.chan = SENSOR_CHAN_ACCEL_XYZ,
							      .type = SENSOR_TRIG_MOTION};

static struct sensor_trigger adxl362_sensor_trigger_stationary = {.chan = SENSOR_CHAN_ACCEL_XYZ,
								  .type = SENSOR_TRIG_STATIONARY};

#if defined(CONFIG_EXTERNAL_SENSORS_IMPACT_DETECTION)
static struct sensor_trigger adxl372_sensor_trigger = {.chan = SENSOR_CHAN_ACCEL_XYZ,
...
#if defined(CONFIG_EXTERNAL_SENSORS_IMPACT_DETECTION)
static void impact_trigger_handler(const struct device *dev, const struct sensor_trigger *trig)
{
	struct sensor_value data[ACCELEROMETER_CHANNELS];
	struct ext_sensor_evt evt = {0};
	int err;
...
int ext_sensors_init(ext_sensor_handler_t handler)
{
	...
	if (!device_is_ready(accel_sensor_hg.dev)) {
		...
		int err = sensor_trigger_set(accel_sensor_hg.dev, &adxl372_sensor_trigger,
					     impact_trigger_handler);
		if (err) {
...
int ext_sensors_temperature_get(double *ext_temp)
{
	...
	if (err) {
		LOG_ERR("Failed to fetch data from %s, error: %d", temp_sensor.dev->name, err);
		evt.type = EXT_SENSOR_EVT_TEMPERATURE_ERROR;
		evt_handler(&evt);
		return -ENODATA;
	}
...
int ext_sensors_humidity_get(double *ext_hum)
{
	...
	if (err) {
		LOG_ERR("Failed to fetch data from %s, error: %d", humid_sensor.dev->name, err);
		evt.type = EXT_SENSOR_EVT_HUMIDITY_ERROR;
		evt_handler(&evt);
		return -ENODATA;
	}
...
int ext_sensors_pressure_get(double *ext_press)
{
	...
	if (err) {
		LOG_ERR("Failed to fetch data from %s, error: %d", press_sensor.dev->name, err);
		evt.type = EXT_SENSOR_EVT_PRESSURE_ERROR;
		evt_handler(&evt);
		return -ENODATA;
	}
...
int ext_sensors_air_quality_get(uint16_t *ext_bsec_air_quality)
{
	...
	if (err) {
		LOG_ERR("Failed to fetch data from %s, error: %d", iaq_sensor.dev->name, err);
		evt.type = EXT_SENSOR_EVT_AIR_QUALITY_ERROR;
		evt_handler(&evt);
		return -ENODATA;
	}
...
int ext_sensors_accelerometer_threshold_set(double threshold, bool upper)
{
	...
	if ((threshold > range_max_m_s2) || (threshold <= 0.0)) {
		LOG_ERR("Invalid %s threshold value: %f", upper ? "activity" : "inactivity",
			threshold);
		return -ENOTSUP;
	}
	 ...
	 */
	threshold = (threshold * (ADXL362_THRESHOLD_RESOLUTION_DECIMAL_MAX / range_max_m_s2));
	...
	const struct sensor_value data = {.val1 = input_value};
	...
	err = sensor_attr_set(accel_sensor_lp.dev, SENSOR_CHAN_ACCEL_X, attr, &data);
	if (err) {
...
int ext_sensors_inactivity_timeout_set(double inact_time)
{
	...
	const struct sensor_value data = {.val1 = inact_time_decimal};

	err = sensor_attr_set(accel_sensor_lp.dev, SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_HYSTERESIS,
			      &data);
	if (err) {
...
int ext_sensors_accelerometer_trigger_callback_set(bool enable)
{
	...
	return 0;
error:
	LOG_ERR("Could not set trigger for device %s, error: %d", accel_sensor_lp.dev->name, err);
	evt.type = EXT_SENSOR_EVT_ACCELEROMETER_ERROR;
	evt_handler(&evt);
	return err;
}
